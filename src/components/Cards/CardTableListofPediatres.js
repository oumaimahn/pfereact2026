...
export default function CardTableListofPediatres({ color }) {
    ...
    const getPediatres = async () => {
        try {
            ...
            const pediatresList = allUsers.filter(user => user.role === "pediatre");
            setPediatres(pediatresList);
        } catch (error) {
            console.log("Erreur lors du chargement des pédiatres", error);
        }
    };
    ...
    const validerPediatre = async (id) => {
        try {
            await axios.put(`http://localhost:5000/users/validerPediatre/${id}`);
            getPediatres();
        } catch (error) {
    ...
    const refuserPediatre = async (id) => {
        const reason = prompt("Raison du refus :");
        if (!reason) return;
        try {
            await axios.put(`http://localhost:5000/users/refuserPediatre/${id}`, { reason });
            getPediatres();
        } catch (error) {
        ...
        <>
            ...
            >
                ...
                {/* <input
                    type="text"
                    placeholder="Rechercher par nom ou email..."
                    className="border-0 px-3 py-2 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-64 ease-linear transition-all duration-150"
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                /> */}
                            ...
                            {filteredPediatres.map((pediatre) => (
                                <tr key={pediatre._id}>
                                    ...
                                    <th className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left flex items-center">
                                        <img
                                            src={`http://localhost:5000/images/users/${pediatre.image_User}`}
                                    ...
                                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        {pediatre.justificatif ? (
                                            <a
                                                ...
                                                href={`http://localhost:5000/images/users/${pediatre.justificatif}`}
                                                target="_blank"
                                    ...
                                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        <span className={
                                            pediatre.accountStatus === "actif" ? "text-green-500 font-bold" :
                                                pediatre.accountStatus === "refuse" ? "text-red-500 font-bold" :
                                    ...
                                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        <button
                                            className="bg-green-500 text-white font-bold uppercase text-xs px-3 py-1 rounded shadow hover:shadow-md mr-1"
                                            onClick={() => validerPediatre(pediatre._id)}
                                        >
                                            Valider
                                        </button>
                                        <button
                                            className="bg-red-500 text-white font-bold uppercase text-xs px-3 py-1 rounded shadow hover:shadow-md"
                                            onClick={() => refuserPediatre(pediatre._id)}
                                        >
                                            Refuser
                                        </button>
                                    </td>
...
CardTableListofPediatres.defaultProps = {
    color: "dark",
};

CardTableListofPediatres.propTypes = {
